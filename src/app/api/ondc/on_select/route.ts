...
// responses.ts. `context` is the verified payload.context, threaded in by the
// handler once it has parsed one.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return nack(500, coreError("could not store quote"), trace, ctx);
  }
