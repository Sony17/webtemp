...
export async function POST(req: Request) {
  ...
  if (isWorkbenchVerificationBypass(parsed.subscriberId)) {
    console.warn("ondc.on_init workbench signature-verification bypass", {
  ...
  if (!fresh.ok) {
    console.warn("ondc.on_init freshness rejected", {
  ...
  if (seen.status === "replay") {
    console.log("ondc.on_init idempotent replay", {
      transactionId: result.data.transactionId,
      messageId: result.data.messageId,
